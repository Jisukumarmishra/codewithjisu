        ...
        <nav className="nav-links">
          <ul>
            {NAV_LINKS.map(({ label, href }) => {
              // Current page check kar rahe hain
              ...
              return (
                <li key={href}>
                  <Link
                    href={href}
                    // Agar isActive true hai, to 'active' class add ho jayegi
                    className={`nav-btn ${isActive ? "active" : ""}`}
                    style={{
                      display: "inline-block",
                      color: isActive ? "#3b82f6" : "inherit", // Active hone par blue color
                      fontWeight: isActive ? "bold" : "normal",
                    }}
                  >
                    <span>{label}</span>
                    {/* Ye underline sirf active link par ya hover par dikhegi */}
                    <span
